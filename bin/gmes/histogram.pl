...
# set pseudo in empty input - uniform
...
if ( $cfg->{$section}->{'unipseudo'} > 0 )
{
	AddUniPseudo( $cfg->{$section}->{'unipseudo'}, \%counts, $min_, $max_ );
}
...
sub AddUniPseudo
{
	...
	my $total_pseudo = int( $pct * $total / 100 );
	my $range = $maxR - $minL + 1;
	srand(1);

	for( 1..$total_pseudo)
	{
		my $pseudo = int(rand($range)) + $minL;
...
sub FilterCounts
{
	...
	if( ! scalar keys %{$ref_out} )
	{
		print "warning, no data in specified range $0\n";
	}
...
#	if ( $cfg->{$section}->{'unipseudo'} > 100)
#		{ die "error, out of range parameter value unipseudo: $cfg->{$section}->{'unipseudo'}\n"; }
}
# ------------------------------------------------
sub ParseCMD
{
...
optional:
  ...
  --unipseudo [%] add pseudocounts of random uniform type; as % from total counts
