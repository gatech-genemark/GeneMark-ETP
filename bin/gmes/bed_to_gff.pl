...
#
# This script takes as input file with junctions reported by TopHap in BED format
# and outputs intron coordinates in GFF format
#
# See details about TopHat output at http://ccb.jhu.edu/software/tophat/index.shtml
# ------------------------------------------------
...
sub LoadSeqToHash
{
	...
	while( my $line = <$IN>)
	{
		...
		if ( $line =~ /^\s*>/ )
		{
			...
			else { die "unexpected defline format found: $line\n"; }
...
sub Usage
{
	print qq(
# -----
usage $0  --bed [filename]  --gff [filename]  --label [label]

required:
  --bed    [filename/s]; input junctions.bed files from RNA-Seq alignment
  --gff    [filename]; output intron coordinates in GFF format

optional:
  --label  [$label]; label for 'source' field (column two) in GFF file
  --seq    [filename]; file with sequence
  --v      verbose
# -----
);
	exit 1;
