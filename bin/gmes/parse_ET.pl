...
my %h;
LoadFromGFF( $cfg->{$section}->{'introns_in'}, \%h, uc 'intron', $cfg->{$section}->{'score'} );
...
{
	Transition_I_vs_T( $cfg->{$section}->{'tr_i_vs_t'}, $cfg->{$section}->{'set_in'}, $min_cds );
}
...
sub Transition_I_vs_T
{
	...
	# number of introns in gene => number of such genes	
	...
	# empty input - stop
	if ( ! keys %hash_introns_genes )
	{
		die "eror, input has no gene information in $0: $name_in\n";
	}
	...
	foreach my $key ( sort{$a<=>$b} keys %hash_introns_genes )
	{
		...
		$sum_obs += ( $key * $hash_introns_genes{$key} );
		$count_obs += $hash_introns_genes{$key};
	}
	$value1 = 1 - $count_obs/$sum_obs;
	...
	{
		if( $key == 0 or $hash_introns_genes{$key} == 0 )
		{
	...
	print "genes in IvsT all and selected: $sum_obs $sum\n" if $v;
	...
	if ( $count > 2 )
	{
		...
		if ( scalar @x > 2 )
		{
			...
			if (defined $lineFit->rSquared())
			{
				...
				if ( $value2 > 1 )
				{
					print "warning, line fit in IvsT hit the max: $value2\n";
					$value2 = 0.9;
				}
			}
		}
...
sub SplitSet
{
	...
	if( $cfg->{$section}->{'intergenic_len'} )
	{
		$name = File::Spec->catfile( $cfg->{$section}->{'parse_dir'}, $cfg->{$section}->{'intergenic_len'} );
		open( $INTERGENIC_LEN, ">", $name ) or die"error on open file $0: $name\n";
	}
	...
	while( my $line = <$PARSE> )
	{
		if( $line =~ /^\#\s*(\S+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+([.+-])\s+(\d+)\s+(\d+)\s+(\S+)\s*$/ )
		{
			...
			if( $INTERGENIC_LEN and ( $label eq 'Intergenic') )
			{
				print $INTERGENIC_LEN  "$len\n";
			}
	...
	close $INTERGENIC_LEN if $INTERGENIC_LEN;
