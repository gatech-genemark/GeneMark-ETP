...
# new scoring rules
my $score_coverage = 0;
my $score_alignment = 0;

($score_coverage, $score_alignment) = ParseScore( $cfg->{$section}->{'score'} );

$cfg->{$section}->{'score_coverage'} = $score_coverage;
$cfg->{$section}->{'score_alignment'} = $score_alignment;
...
my %h;
LoadFromGFF( $cfg->{$section}->{'introns_in'}, \%h, 'INTRON', $score_coverage, $score_alignment );
...
sub ParseScore
{
	...
	{
		print "error, unexpected format found in score value: $value\n";
		exit 1;
	}
...
sub LoadFromGFF
{
	my( $name, $ref, $label, $coverage, $alignment ) = @_;
	...
	while( my $line = <$GFF> )
	{
		...
		if ( $line =~ /^(\S+)\t(\S+)\t(\S+)\t(\d+)\t(\d+)\t(\S+)\t(\S+)\t(\S+)\t(.+)/ )
		{
			...
			if( $tmp{'score'} ne '.' and  $tmp{'score'} < $coverage ) { next; }

			if ( $tmp{'attr'} =~ /al_score=/ )
			{
				if ( $tmp{'attr'} =~ /al_score=([^;]+)/ )
				{
					if ( $1 < $alignment ) {next;}
...
required:
 ...
 optional:
  ...
  --score      [number,number] filter out introns with scores below threshold
